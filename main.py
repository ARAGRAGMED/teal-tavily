...
def root():
    ...
    return {"message": "Tavily FastAPI backend is running."}

# API endpoint for chatbot UI
@app.post("/api/search")
def api_search(request: SearchRequest):
    query = request.message
    language = request.language
    print(f"API search called with query: {query}, language: {language}")
    if not query:
        print("Error: No message provided")
        return JSONResponse(status_code=400, content={"error": "No message provided"})
    tavily_response = search_tavily(query, language)
    print(f"Tavily response: {tavily_response}")
    if tavily_response.get('results'):
        print(f"First result keys: {list(tavily_response['results'][0].keys())}")
        print(f"First result published_date: {tavily_response['results'][0].get('published_date')}")
    results = []
    for result in tavily_response.get('results', [])[:RESULT_LIMIT]:
        results.append({
            'title': result.get('title', ''),
            'snippet': result.get('content', ''),  # Tavily API uses 'content' not 'snippet'
            'source': result.get('url', ''),
            'date': result.get('published_date', '')
        })
    print(f"Extracted results: {results}")
    # Use OpenAI to generate a detailed answer
